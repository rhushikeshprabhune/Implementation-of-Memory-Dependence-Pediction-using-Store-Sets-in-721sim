...
void pipeline_t::dispatch() {
   ...
   for (i = 0; i < dispatch_width; i++) {
      ...
      if (IS_MEM_OP(PAY.buf[index].flags)) {
         ...
         if(MEM_DEP_PRED == 1)
         {
            if(MDP.find(PAY.buf[index].pc) != MDP.end())
            {
               stall = true;
            }
         }
         ...
         if(MEM_DEP_PRED == 2)
         {
            if(IS_LOAD(PAY.buf[index].flags))
            {
               ...
               int retValue = storeSet.lookup_ssit(PAY.buf[index].pc);
               if(retValue != -1)
               {
                  int retLFST = storeSet.lookup_lfst(retValue);
                  if(retLFST == 0)
                  {
                     stall = false;
                  }
            ...
            if(IS_STORE(PAY.buf[index].flags))
            {
               ...
               if(retValue != -1)
               {
                  ...
                  {
                     stall = true;
                     storeSet.update_lfst(PAY.buf[index].pc, PAY.buf[index].AL_index + ACTIVE_LIST_SIZE);
                  }
               }
            }
         }
