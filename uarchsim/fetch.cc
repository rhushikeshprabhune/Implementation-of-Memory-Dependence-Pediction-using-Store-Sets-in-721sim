	 ...
	 fetch_exception = true;
      ...
      switch (insn.opcode()) {
	    ...
	    if (PERFECT_BRANCH_PRED) {
         ...
         default:
            next_pc = INCREMENT_PC(pc);   // May get modified in corner-case below.
	    if (pred_valid) {
	       if (!fetch_exception) {
	       ...
	       else if ((branch_vector & (1 << i)) != 0) {
		  printf("*** WARNING: fetch exception on a branch already trained in the BTB (and possibly TCM).\n");
		  if (i == (fetch_bundle_length - 1))
	             next_pc = next_fetch_pc;
	       }
	    }
