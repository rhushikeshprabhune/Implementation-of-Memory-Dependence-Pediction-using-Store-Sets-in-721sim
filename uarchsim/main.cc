...
static void help()
{
  ...
  fprintf(stderr, "  --disambig=<oracle>,<spec>,<mdp>,<cylic_clear>\tEach of <oracle> (oracle memory disambig.), <spec> (speculative memory disambig.), and <mdp> (mem. dep. predictor), are 0 or 1\n");
...
static void set_disambig_flags(const char* config) {
   uint64_t omd, smd, mdp, cylic_clear;
   if (sscanf(config, "%lu,%lu,%lu,%lu", &omd, &smd, &mdp, &cylic_clear) != 4) {
      fprintf(stderr, "Incorrect usage of --disambig=<oracle>,<spec>,<mdp>,<cylic_clear>\n");
      fprintf(stderr, "...where oracle (oracle memory disambig.), spec (speculative memory disambig.), and mdp (mem. dep. predictor), are each 0 or 1 & cylic_clear is integer\n");
      exit(-1);
   }
   else {
