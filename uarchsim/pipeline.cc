  ...
  LSU(lq_size, sq_size, Tid, _mmu, this)
{
  ...
  if(MEM_DEP_PRED == 2)
  {
    fprintf(stats_log, "   USE STORE-SET MEMORY DEPENDENCE PREDICTOR = %d\n", MEM_DEP_PRED);
  }
  if(MEM_DEP_PRED)
  {
    fprintf(stats_log, "   USE CYCLIC_CLEAR INTERVAL FOR MEMORY DEPENDENCE PREDICTOR = %d\n", CYCLIC_CLEAR);
  }
  ...
  try
  {
    ...
    if (unlikely(debug))
    {
      ...
      while (instret < n)
      {
        ...
        // Pipeline.
        /////////////////////////////////////////////////////////////
        // Cyclic-clear of Sticky Bit Table
        if((MEM_DEP_PRED) && (CYCLIC_CLEAR != 0))
        {
            if(((num_insn % CYCLIC_CLEAR) == 0) && (CYCLIC_CLEAR != 0))
            {
                MDP.clear();
                storeSet.ssit.clear();
                storeSet.lfst.clear();
...
void pipeline_t::set_store_pc(uint64_t store_pc, unsigned int AL_index) {
    REN->set_store_pc(store_pc, AL_index);
}

void pipeline_t::set_store_inum(unsigned int AL_index, unsigned int store_inum) {
    REN->set_store_inum(AL_index, store_inum);
}
