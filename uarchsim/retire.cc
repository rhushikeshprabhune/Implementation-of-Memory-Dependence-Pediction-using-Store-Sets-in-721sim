...
void pipeline_t::retire(size_t& instret) {
   ...
   head_valid = REN->precommit(completed,
                               exception,
                               load_viol,
                               br_misp,
                               val_misp,
   ...
   if (head_valid && completed) {    // AL head exists and completed
    ...
    // This is a mispredicted load owing to speculative memory disambiguation (not value prediction).
    // Therefore the load is incorrect and not committed.
      // If the store-set memory dependence predictor is enabled,
      // add the offending load to the predictor.
   if(MEM_DEP_PRED == 2) {
      storeSet.update_ssit(offending_PC, offendStorePC);
      storeSet.update_lfst(offending_PC, offendStoreALIndex + ACTIVE_LIST_SIZE);
      storeSet.update_lfst(offendStorePC, offendStoreALIndex + ACTIVE_LIST_SIZE);
	 }
